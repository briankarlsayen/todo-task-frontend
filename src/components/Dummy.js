...
function Dummy() {
    ...
    const load = async () => {
        ...
        .then(res => {
            ...
            setTodoList(res.data[0].list.reverse())
            setLoading(false)
            setDone(true)
        })
        .catch(err => {
                ...
                <div className="todoList__body">
                    ...
                    <div>
                        <form className="todoList__form" onSubmit={handleSubmit}>
                            <input className="todoList__txt" required type="text" placeholder='Add todo' 
                            value={addTodoList} onChange={e => setAddTodoList(e.target.value)} />
                        </form>
                    </div>
